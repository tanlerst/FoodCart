...
export default function FoodRecommendationPage({ recommendedDishes, onAddDish }: FoodRecommendationPageProps) {
    ...
    // if no recommended dishes

    const currentDish = recommendedDishes[currentDishIndex];

    function moveToNextDish() {
        if (currentDishIndex < recommendedDishes.length - 1) {
            setCurrentDishIndex(currentDishIndex + 1);
        } else {
            // last dish
            setCurrentDishIndex(0); // Loop back to the first dish
        }
    }
