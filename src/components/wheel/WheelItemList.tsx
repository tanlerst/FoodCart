...
export default function WheelItemList({ items, onRemove, onClear }: WheelItemListProps) {
  return (
    <section className="mt-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between">
        <h2 className="font-bold text-gray-900">Your Items ({items.length})</h2>
      ...
      <div className="mt-2">
        {/* Wheel items */}
        {items.map((item) => (
          <WheelItemRow key={item.id} item={item} onRemove={onRemove} />
        ))}
      </div>
    </section>
  );
